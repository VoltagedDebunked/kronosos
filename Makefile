...
# Target architecture to build for. Set to x86_64.
$(call USER_VARIABLE,ARCH,x86_64)

# Default user QEMU flags. These are appended to the QEMU command calls.
$(call USER_VARIABLE,QEMUFLAGS,-m 2G)

override IMAGE_NAME := KronosOS-$(ARCH)
...
run-x86_64: ovmf/ovmf-code-$(ARCH).fd $(IMAGE_NAME).iso
	qemu-system-$(ARCH) \
		...
		-drive id=disk,if=none,format=raw,file=img/ata.img \
...
$(IMAGE_NAME).iso: limine/limine kernel
	...
	cp -v kernel/bin-$(ARCH)/KronosOS-Core iso_root/boot/
...
$(IMAGE_NAME).hdd: limine/limine kernel
	...
	mcopy -i $(IMAGE_NAME).hdd@@1M kernel/bin-$(ARCH)/KronosOS-Core ::/boot
